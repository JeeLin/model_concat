...
#[derive(Debug)]
pub struct AnthropicWebsocketHandler {
    api_key: String,
    connected: bool,
}

impl AnthropicWebsocketHandler {
...
impl WsHandler for AnthropicWebsocketHandler {
    fn connect(&self, url: &str, headers: &[(String, String)]) -> Result<(), AdapterError> {
        ...
        rt.block_on(async {
            debug!("尝试建立Anthropic WebSocket连接");
    ...
    fn send_message(&self, message: &[u8]) -> Result<(), AdapterError> {
        ...
        // 构造带有认证头的WebSocket请求
        let auth_header = format!("X-API-Key: {}", &self.api_key);
        let mut request = vec![];
    ...
    fn receive_message(&self) -> Result<Vec<u8>, AdapterError> {
        ...
        warn!("Anthropic WebSocket处理器尚未完全实现，返回空响应");
        Ok(Vec::new())
    }
...
impl ProtocolHandler for AnthropicWebsocketHandler {
    fn send_request(&self, payload: &[u8]) -> Result<Vec<u8>, AdapterError> {
        // 连接WebSocket
        self.connect("wss://api.anthropic.com/v1/chat/completions", &[])?;
