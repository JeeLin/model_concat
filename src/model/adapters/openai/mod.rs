//! OpenAI模型适配器模块
//!
//! 本模块提供了对OpenAI API服务的适配支持，包括HTTP和WebSocket两种协议的处理器。
//! 支持GPT系列模型的调用和管理。
...
impl OpenAIAdapter {
    ...
    pub fn validate(&self) -> Result<(), AdapterError> {
        ...
        if self.connection_config.api_key.is_empty() {
            return Err(AdapterError::ConnectionError("OpenAI API密钥不能为空".to_string()));
        }
        
        if self.connection_config.base_url.is_empty() {
            return Err(AdapterError::ConnectionError("OpenAI 服务基础URL不能为空".to_string()));
        }
        ...
        match self.connection_config.protocol_type {
            ProtocolType::Http => {
                // 创建HTTP处理器并测试连接
                let handler = http::OpenAIHttpHandler::new(
            ...
            ProtocolType::WebSocket => {
                // 创建WebSocket处理器并测试连接
                let handler = websocket::OpenAIWebsocketHandler::new(
                    self.connection_config.api_key.clone(),
                );
...
impl ProviderAdapter for OpenAIAdapter {
    ...
    fn select_protocol(
        ...
        match protocol {
            ProtocolType::Http => Ok(Box::new(http::OpenAIHttpHandler::new(
                self.connection_config.api_key.clone(),
                self.connection_config.base_url.clone(),
            ))),
            ProtocolType::WebSocket => Ok(Box::new(websocket::OpenAIWebsocketHandler::new(
                self.connection_config.api_key.clone(),
            ))),
        }
    }
    ...
    ///
    /// # 参数
    /// * `model_id` - 模型ID，如"gpt-3.5-turbo"或"gpt-4"
    /// * `parameters` - 模型参数配置
    ///
    /// # 返回
    ...
    fn create_model(
        ...
        match model_id {
            "gpt-3.5-turbo" | "gpt-4" => {
                // 创建ChatGPT模型实例
                Ok(Box::new(http::OpenAIChatModel::new(
                    model_id.to_string(),
                    self.connection_config.api_key.clone(),
                    self.connection_config.base_url.clone(),
                    parameters.clone(),
